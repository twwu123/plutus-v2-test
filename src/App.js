...
function App() {
  ...
  const testSend = async () => {
    ...
    wasmMultiasset.insert(wasm.ScriptHash.from_bytes(hexToBytes("a07d5059b1e1371f2afc4d3356c69733e04f0aea2f1a75cfd1822ced")), wasmAssets)
  ...
  const testRedeem = async () => {
    ...
    const transactionHash = "4a322d7e2387aafa9ff13b6f574321dc2e9bee07ae92a8fe8757c1bd34c5ea7e"
    const outputId = "0"
    ...
    const wasmData = jsonDataToWasmDatum({
      "fields": [],
      "constructor": 0
    })
    ...
    const wasmRedeemer = wasm.Redeemer.new(
      ...
      wasmData,
      wasm.ExUnits.new(
        wasm.BigNum.from_str("842996"),
        wasm.BigNum.from_str("246100241")
      )
    )
    ...
    wasmOutput.set_plutus_data(wasmData)
    txBuilder.add_output(wasmOutput)

    // We need to handle hashing of plutus witness. This is quite annoying currently, because the datum is actually included inline within the script UTXO
    // therefore, we need to intentionally leave out the datum in the witness set.
    ...
    const plutusWitnessHash = wasm.hash_script_data(wasmRedeemers, wasm.TxBuilderConstants.plutus_vasil_cost_models(), undefined)
